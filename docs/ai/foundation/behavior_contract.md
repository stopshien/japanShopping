...
- **The tax multiplier follows the currency, not the user.** Both Japanese yen and Korean won currently use 1.1. There is no separate tax-rate picker; choosing a currency chooses its country's rate. The rate stays attached to the currency so a future change to either country, or a new currency, needs no change to the calculation. `Currency.taxMultiplier`, `PriceBreakdownTests`.
- **Japan uses 10%, its standard consumption tax rate.** Japan also has an 8% reduced rate for food and drink, which this app does not apply. Grocery purchases will therefore show slightly more tax than actually charged. Supporting both would need a per-purchase rate choice.
