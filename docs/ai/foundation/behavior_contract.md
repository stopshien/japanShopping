...
- **The compute screen reloads the currency through `reloadSettings()`** when Settings reports it saved, rather than re-reading on every appearance. A currency change clears the typed amount and the result, because the typed number was a price in the old currency.
