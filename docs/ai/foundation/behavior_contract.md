...
- **A shopping list row is two lines with the amount on the right.** Name on the first line, pay type (a tinted tag) and tax state on the second, amount right-aligned in monospaced digits so the column lines up and can be scanned down. The name is not a large font: it competes with the amount and made every row three lines tall. The thumbnail is 56pt and is hidden entirely when the item has no photo. `ShoppingListViewModelTests`.
- **The bottom summary sits in a floating card**, like the lists and the other screens, instead of resting directly on the green background.
- **An empty list shows what to do next** (「還沒有任何消費紀錄」) rather than only `NT$ 0`.
