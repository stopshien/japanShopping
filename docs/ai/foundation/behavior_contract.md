...
- **The shopping list opened by 加入購物清單 has no way back.** The back button is hidden and the swipe-back gesture is disabled, so 完成 is the only exit and it returns to the compute screen. Going back to the detail screen would allow saving the same item twice. The list opened from the list buttons on the compute and detail screens still allows going back. `DetailViewModelTests`.
